...
"""
code to evaluate already generated INTERACTIVE explanations from ontology-based narratives using language models 
(in this code, an agent from pydantic-ai is used)

a simulated user asked for shorter and clearer explanations, we evaluate here the shorter ones
"""
...
if __name__ == "__main__":
    ...
    explanations_csv_file = "generated_c_interactive_narratives_multiple_plans_comparison_with_specificity_" \
        + str(specificity) + "_dataset_" + dataset_name + "_using_language_models_" + model_used_to_generate_explanations + ".csv"
    evaluation_results_file = "quantitative_interactive_evaluation_results_multiple_plans_comparison_with_specificity_" \
    ...
    results_dict = {"Pair ID": list(), "Dataset" : list(), "Language model" : list(), "Specificity" : list(), \
                     "Words": list(), "Words shorter": list(), \
                     "Readability score": list(), "Readability score shorter": list(), \
                     "Cosine similarity": list(), "Cosine similarity shorter": list(), \
                     "Bi-encoder similarity": list(), "Bi-encoder similarity shorter": list(), \
                     "Cross-encoder similarity": list(), "Cross-encoder similarity shorter": list(), \
                     "Cross-encoder inference": list(), "Cross-encoder inference shorter": list(),}
    ...
    for i in range (0, number_of_explanations):    
        ...
        results_dict['Words shorter'].append(len(explanations_dict['Explanation shorter'][i].split()))
        ...
        results_dict['Readability score shorter'].append(textstat.flesch_reading_ease(explanations_dict['Explanation shorter'][i]))
        ...
        results_dict['Cosine similarity shorter'].append(test_utils_object.compute_text_cosine_similarity(\
            [narratives_dict["Explanation"][i], explanations_dict['Explanation shorter'][i]])) # item() turns the np.float into python float
        ...
        results_dict['Bi-encoder similarity shorter'].append(bi_encoder_model.similarity(\
            bi_encoder_model.encode(narratives_dict["Explanation"][i]), \
            bi_encoder_model.encode(explanations_dict['Explanation shorter'][i])).item()) # cosine-similarity by default (item is used to extract the value of the result, which is a tensor)
        results_dict['Cross-encoder similarity'].append(cross_encoder_model_for_text_similarity.predict(\
            [narratives_dict["Explanation"][i], explanations_dict['Explanation'][i]]))
        results_dict['Cross-encoder similarity shorter'].append(cross_encoder_model_for_text_similarity.predict(\
            [narratives_dict["Explanation"][i], explanations_dict['Explanation shorter'][i]]))

        cross_encoder_inference_result_scores = cross_encoder_model_for_natural_language_inferece.predict(\
            [narratives_dict["Explanation"][i], explanations_dict['Explanation'][i]])
        ...
        cross_encoder_inference_result_scores_shorter = cross_encoder_model_for_natural_language_inferece.predict(\
            [narratives_dict["Explanation"][i], explanations_dict['Explanation shorter'][i]])
        cross_encoder_inference_result_label_shorter = cross_encoder_model_for_natural_language_inferece_label_mapping[np.argmax(cross_encoder_inference_result_scores)]
        results_dict['Cross-encoder inference shorter'].append(cross_encoder_inference_result_label_shorter)
