...
if __name__ == "__main__":
    ...
    enhanced_narratives_csv_file = "generated_c_interactive_narratives_multiple_plans_comparison_with_specificity_" \
        + str(specificity) + "_dataset_" + dataset_name + "_using_language_models_" + model_to_run + ".csv"
    evaluation_results_file = "interactive_evaluation_results_multiple_plans_comparison_with_specificity_" \
        + str(specificity) + "_dataset_" + dataset_name + "_using_language_models_" + model_to_run + ".csv"
    ...
    enhanced_narratives_dict['Explanation shorter'] = list()
    enhanced_narratives_dict['Explanation clearer'] = list()
    enhanced_narratives_dict['Modifications'] = list()
    enhanced_narratives_dict['Modifications shorter'] = list()
    enhanced_narratives_dict['Modifications clearer'] = list()
    enhanced_narratives_dict['Language model'] = list()

    results_dict = {"Pair ID": list(), "Dataset" : list(), "Language model" : list(), "Specificity" : list(), \
                     "Words": list(), "Words shorter": list(), "Words clearer": list(), \
                     "Readability score": list(), "Readability score shorter": list(), "Readability score clearer": list(), \
                     "Cosine similarity": list(), "Cosine similarity shorter": list(), "Cosine similarity clearer": list()}
    ...
    for i in range (0, number_of_explanations):    
        ...
        interactive_result_shorter = explanation_generation_agent.run_sync(  
            'Make the explanation shorter.', message_history=same_history_as_step_1
        )

        interactive_result_clearer = explanation_generation_agent.run_sync(  
            'Make the explanation clearer.', message_history=same_history_as_step_1
        )
        ...
        enhanced_narratives_dict['Explanation shorter'].append(interactive_result_shorter.output.narrative)
        enhanced_narratives_dict['Explanation clearer'].append(interactive_result_clearer.output.narrative)
        enhanced_narratives_dict['Modifications'].append(result.output.modifications)
        enhanced_narratives_dict['Modifications shorter'].append(interactive_result_shorter.output.modifications)
        enhanced_narratives_dict['Modifications clearer'].append(interactive_result_clearer.output.modifications)
        ...
        results_dict['Words shorter'].append(len(interactive_result_shorter.output.narrative.split()))
        results_dict['Words clearer'].append(len(interactive_result_clearer.output.narrative.split()))
        results_dict['Readability score'].append(textstat.flesch_reading_ease(result.output.narrative))
        results_dict['Readability score shorter'].append(textstat.flesch_reading_ease(interactive_result_shorter.output.narrative))
        results_dict['Readability score clearer'].append(textstat.flesch_reading_ease(interactive_result_clearer.output.narrative))
        ...
        results_dict['Cosine similarity shorter'].append(test_utils_object.compute_text_cosine_similarity(\
            [narratives_dict["Explanation"][i], interactive_result_shorter.output.narrative])) # item() turns the np.float into python float
        results_dict['Cosine similarity clearer'].append(test_utils_object.compute_text_cosine_similarity(\
            [narratives_dict["Explanation"][i], interactive_result_clearer.output.narrative])) # item() turns the np.float into python float
    ...
    print("SHORTER: ", results_dict["Words shorter"])
    print("CLEARER: ", results_dict["Words clearer"])

    print("\n  Narrative readability (score): ", results_dict["Readability score"]) 
    print("SHORTER: ", results_dict["Readability score shorter"]) 
    print("CLEARER: ", results_dict["Readability score clearer"]) 

    print("\n  Narrative cosine similary: ", results_dict["Cosine similarity"]) 
    print("SHORTER: ", results_dict["Cosine similarity shorter"]) 
    print("CLEARER: ", results_dict["Cosine similarity clearer"]) 
    print("\n") 